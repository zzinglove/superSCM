-- STEP 6 보정: 월말 train_end와 월초 demand grid의 날짜 기준을 통일한다.
...
declare
  ...
  window_size integer; lag_months integer; target_month date := date_trunc('month', p_period)::date;
begin
  if p_model_id in ('PY_SAME_MONTH','SEASONAL_NAIVE') then
    ...
    select quantity into q1 from pg_temp.tmp_grid
    where item_id=p_item_id and period=(target_month-make_interval(months=>lag_months))::date;
    return q1;
  elsif p_model_id='WMA_3M' then
    select max(quantity) filter(where period=(target_month-interval '1 month')::date),
           max(quantity) filter(where period=(target_month-interval '2 months')::date),
           max(quantity) filter(where period=(target_month-interval '3 months')::date)
      into q1,q2,q3 from pg_temp.tmp_grid
     where item_id=p_item_id and period between (target_month-interval '3 months')::date and (target_month-interval '1 month')::date;
    ...
    select max(quantity) filter(where period=(target_month-interval '1 month')::date),
           max(quantity) filter(where period=(target_month-interval '2 months')::date),
           max(quantity) filter(where period=(target_month-interval '3 months')::date),
           max(quantity) filter(where period=(target_month-interval '4 months')::date),
           max(quantity) filter(where period=(target_month-interval '5 months')::date),
           max(quantity) filter(where period=(target_month-interval '6 months')::date)
      into q1,q2,q3,q4,q5,q6 from pg_temp.tmp_grid
     where item_id=p_item_id and period between (target_month-interval '6 months')::date and (target_month-interval '1 month')::date;
...
begin
  ...
  if setting.train_start is null or setting.train_end is null or setting.granularity<>'month' or coalesce(setting.forecast_horizon,0)<=0 then raise exception 'monthly forecast setting is incomplete'; end if;
  ...
  begin
    ...
    create index tmp_grid_item_period_idx on tmp_grid(item_id,period); analyze tmp_grid;
